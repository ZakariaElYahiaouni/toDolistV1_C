...
namespace WA_toDolist
{
    public partial class FormMenu : Form
    {
        ...
        private void button1_Click(int rowId, int i)
        {
            ...
            FormEdit form = new FormEdit(rowId, i);
        ...
        private void txt_search_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                connection.Open();
                //where title like '%ul%' or descriptionDuty like '%ul%'

                string query = "SELECT Id, title, descriptionDuty FROM toDoList WHERE title like'%" + (txt_search.Text) + "%' or descriptionDuty like '%"+(txt_search.Text) +"%'";
                dt.Clear();
                SqlCommand commandSearch = new SqlCommand(query, connection);
                SqlDataReader readerSearch;
                readerSearch = commandSearch.ExecuteReader();
